        ...
        loading ? <Loading /> : (
            <section>
                <form onSubmit={submitHandler} className='w-full flex flex-col gap-[4px]'>
                    <label>
                        <span>Name</span>
                        <input value={name} onChange={(e) => setName(e.target.value)} type="text" placeholder='Enter Vendor Name' />
                    </label>

                    <label>
                        <span>Email</span>
                        <input value={email} onChange={(e) => setEmail(e.target.value)} type="text" placeholder='Enter Vendor Email' />
                    </label>

                    <label>
                        <span>Country</span>
                        <Select value={country} onChange={setCountry} options={countriesOptions} styles={styles} />
                    </label>
                    ...
                    <label>
                        ...
                        <div className='flex flex-col gap-[12px]'>
                            {programs.map((program, index) => (
                                <div key={index} className='grid grid-cols-[1fr_1fr_1fr_auto] gap-[8px] items-center'>
                                    {/* Program Select */}
                                    <Select
                                        styles={styles}
                                        value={programOptions.find(p => p.value === program) || null}
                                        options={programOptions}
                                        onChange={(selectedOption) => handleProgramChange(selectedOption, index)}
                                    />
                                    {/* Amount Input */}
                                    <input
                                        type="number"
                                    ...
                                    />
                                    {/* Remove Button */}
                                    <button type="button" onClick={() => removeProgram(index)} className="text-red-500">
                                        ✖
                                    </button>
                                </div>
                            ))}
                            <button type="button" onClick={addProgram} className="primary-btn !bg-text !font-[500]">
                                Add Program
                            </button>
                        </div>
                    </label>
