...
import { createInvoice, getInstallmentsByClient, getInvoiceById, updateInvoice } from '../../../redux/actions/invoice'
...
const UpdateInvoice = () => {
    ...
    const { id } = useParams()
    const { installments, invoice, loading, error, message } = useSelector(state => state.invoice)
    ...
    useEffect(() => {
        ...
        setClient(clientOptions.find((c) => c.value === invoice?.client?._id))
        setCurrency(currencyOptions.find((c) => c.value === invoice?.currency))
        setInvoiceItem(invoiceItemOptions.find((i) => i.label === invoice?.invoiceItem?.title))
        setSalesCommission(trueFalseOptions.find((t) => t.value === invoice?.salesCommission))
        setOperationsHeadCommission(trueFalseOptions.find((t) => t.value === invoice?.operationsHeadCommission))
        setOperationSubOrdinateComission(trueFalseOptions.find((t) => t.value === invoice?.operationsSubOrdinateCommission))
    }, [id]);
