...
import { addVendorPayment, getAllVendors } from "../../../redux/actions/vendor";
...
const AddVendorPayments = () => {
    const [vendor, setVendor] = useState({ value: "", label: "" });
    ...
    const { vendors, error, message, loading } = useSelector(state => state.vendor);
    ...
    useEffect(() => {
        if (vendor.value) {
            const selectedVendor = vendors.find(v => v._id === vendor.value);
            if (selectedVendor) {
                const programOptions = selectedVendor.programs.map(p => ({
                    value: p.program._id,
                    label: `${p.program.country} ${p.program.title}`,
                    amount: p.amount
                }));
                setPrograms([{ program: null, amount: "" }]); // Reset programs on vendor change
    ...
    const handleProgramChange = (index, selectedProgram) => {
        ...
        updatedPrograms[index] = {
            program: selectedProgram?.value || "", // Store only the program ID as a string
            amount: selectedProgram?.amount || "" // Keep amount
        };
    ...
    const submitHandler = (e) => {
        ...
        myForm.append('vendor', vendor.value);
        ...
        dispatch(addVendorPayment(myForm, date))

    };

    useEffect(() => {
        alert(message, error, "/finance/vendor/payments")
    }, [message, error])
    ...
    return (
        loading ? <Loading /> : <section className="w-full">
            <form className="w-full flex flex-col gap-4" onSubmit={submitHandler}>
                <label>
                    <span>Vendor</span>
                    <Select value={vendor} onChange={setVendor} options={vendorOptions} styles={styles} />
                </label>
                ...
                <div className="flex flex-col gap-2">
                    {programs.map((p, index) => (
                        <div key={index} className="grid grid-cols-[1fr_1fr_150px] gap-2 items-center">
                            <Select
                                value={availablePrograms.find(p => p.value === programs[index]?.program) || null}
...
export default AddVendorPayments;
