...
const AddNewProgram = () => {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    disptach(createProgram(country?.label, title, duration, currency?.label, totalCost, deduction, processDuration, jobs, documents, requirements, benefits, timelineProcess, moment.tz('Asia/Karachi').format()))
  };
  ...
  return (
    loading ? <Loading /> : <section className='w-full'>
      <form className='w-full flex flex-col gap-[4px]' onSubmit={submitHandler}>
        <label>
          <span>Country</span>
          <Select options={options} styles={styles} placeholder='Choose Country' onChange={changeHandler} />
        </label>
        ...
        <label>
          <span>Currency</span>
          <Select options={currencyOptions} styles={styles} placeholder='Choose Currency' onChange={setCurrency} />
        </label>
